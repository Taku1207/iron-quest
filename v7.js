...
function load(){
 try{
  ...
  starters.forEach(id=>{if(!s.inventory[id])s.inventory[id]={owned:true,upgrade:0}});
 ...
 }catch(e){const s=clone(initial);starters.forEach(id=>s.inventory[id]={owned:true,upgrade:0});return s}
}
...
function scaledValue(v,u){return v?Math.round(v*(1+u*.08)*10)/10:0}
function itemStats(item){const u=itemUpgrade(item.id);return {hp:scaledValue(item.hp,u),atk:scaledValue(item.atk,u),def:scaledValue(item.def,u),crit:scaledValue(item.crit,u)}}
...
function grantDrop(id){if(!id)return "";const it=itemById(id);if(!it)return "";if(!state.inventory[id])state.inventory[id]={owned:true,upgrade:0};return it.name}
...
async function startBattle(){syncBp();if(state.bp<1){toast("BPが足りません。修行または自然回復を待ちましょう");renderBpUI();return}const selectedTier=state.selectedEnemyTier,en={...selectedEnemy()},isFirstClear=!state.clearedAll&&selectedTier===state.enemyTier,cs=combatStats(),tierBefore=state.enemyTier,ov=document.getElementById("battleOverlay");if(!spendBp()){toast("BPが足りません");return}activeBattleEnemy=en;renderBpUI();let ehp=en.hp,php=cs.hp,turn=1;document.getElementById("animPlayerImg").src=stage().img;document.getElementById("animEnemyImg").src=en.img;document.getElementById("animEnemyName").textContent=en.name;document.getElementById("animHpName").textContent=en.name;document.getElementById("animHpText").textContent=en.hp.toLocaleString()+" / "+en.hp.toLocaleString();document.getElementById("animHpFill").style.width="100%";document.getElementById("animPlayerHpText").textContent=cs.hp.toLocaleString()+" / "+cs.hp.toLocaleString();document.getElementById("animPlayerHpFill").style.width="100%";document.getElementById("rewardPanel").classList.remove("show");document.getElementById("battleMessage").textContent="BP -1　BATTLE START";ov.classList.add("show");ov.setAttribute("aria-hidden","false");await wait(500);while(ehp>0&&php>0){const ph=playerAttack(cs,en);ehp=Math.max(0,ehp-ph.dmg);await animateHit("player",ph,ehp,en.hp,php,cs.hp,turn);if(ehp<=0)break;const eh=enemyAttack(en,cs);php=Math.max(0,php-eh.dmg);await animateHit("enemy",eh,ehp,en.hp,php,cs.hp,turn);turn++;if(turn>40){php=0;break}}
 const won=ehp<=0;const oldLv=state.level,powerBefore=power();let goldDelta=0,expDelta=0,drop="—";if(won){goldDelta=isFirstClear?en.gold:Math.max(10,Math.round(en.gold*(.65+Math.random()*.2)));expDelta=isFirstClear?en.exp:Math.max(1,Math.round(en.exp*.25));state.gold+=goldDelta;state.kills++;if(isFirstClear){drop=grantDrop(en.dropItem)||en.drop}else if(Math.random()<.15){drop=grantDrop(en.dropItem)||en.drop}else drop="ドロップなし";addExp(expDelta);if(isFirstClear){if(state.enemyTier<enemies.length-1){state.enemyTier++;state.selectedEnemyTier=state.enemyTier}else{state.clearedAll=true;state.selectedEnemyTier=enemies.length-1}}state.history.unshift({date:new Date().toLocaleDateString("ja-JP"),kind:"BATTLE",result:"WIN",enemy:en.name,gold:goldDelta,exp:expDelta,bp:-1,replay:!isFirstClear,level:state.level})}else{goldDelta=Math.floor(state.gold*.10);state.gold=Math.max(0,state.gold-goldDelta);state.history.unshift({date:new Date().toLocaleDateString("ja-JP"),kind:"BATTLE",result:"LOSE",enemy:en.name,gold:-goldDelta,exp:0,bp:-1,replay:!isFirstClear,level:state.level})}state.history=state.history.slice(0,16);save();renderAll();document.getElementById("rewardTitle").textContent=won?"VICTORY!":"DEFEAT";document.getElementById("rewardTitle").classList.toggle("defeatTitle",!won);document.getElementById("battleResultSub").textContent=won?(isFirstClear?en.name+"を初回撃破！":en.name+"に再戦勝利！"):"所持GOLDの10%を失った";document.getElementById("rewardExp").textContent=won?"+"+expDelta:"+0";document.getElementById("rewardGold").textContent=won?"+"+goldDelta:"-"+goldDelta;document.getElementById("rewardDrop").textContent=won?drop:"—";document.getElementById("battleMessage").textContent=won?(isFirstClear?(tierBefore<enemies.length-1?"次の敵が解放された！":"最終敵を撃破した！"):"再戦報酬を獲得！"):"さらに修行・装備強化して再挑戦しよう";document.getElementById("rewardPanel").classList.add("show");await new Promise(resolve=>{document.getElementById("battleContinue").onclick=()=>{document.getElementById("rewardPanel").classList.remove("show");ov.classList.remove("show");ov.setAttribute("aria-hidden","true");resolve()}});activeBattleEnemy=null;const powerAfter=power();if(won)await playLevelSequence({oldLv,newLv:state.level,powerBefore,powerAfter,muscleUps:[]});go(won&&isFirstClear?"home":"battle")}
document.getElementById("challengeBtn").addEventListener("click",startBattle);
let currentShopFilter="weapon";
function renderGear(){const totals=equipmentTotals();document.getElementById("gearTierBadge").textContent="Tier "+totals.tier;document.getElementById("shopGoldText").textContent=state.gold.toLocaleString();document.getElementById("gearSummary").innerHTML='<div class="gearBonus"><span>HP</span><b>+'+Math.round(totals.hp)+'</b></div><div class="gearBonus"><span>ATK</span><b>+'+Math.round(totals.atk)+'</b></div><div class="gearBonus"><span>DEF</span><b>+'+Math.round(totals.def)+'</b></div><div class="gearBonus"><span>CRIT</span><b>+'+Math.round(totals.crit*10)/10+'%</b></div>';const grid=document.getElementById("equippedGrid");grid.innerHTML="";Object.keys(slotNames).forEach(slot=>{const id=state.equipped[slot],it=itemById(id),z=it?itemStats(it):{hp:0,atk:0,def:0,crit:0},u=it?itemUpgrade(it.id):0;const c=document.createElement("div");c.className="slotCard";c.innerHTML='<div class="small">'+slotNames[slot]+'</div><div class="slotTop"><div class="itemArt art-'+slot+'"></div><div><div class="itemName '+(it?'rarity-'+it.rarity:'')+'">'+(it?.name||"未装備")+(u?" +"+u:"")+'</div><div class="itemStats">'+(it?formatBonus(z):"—")+'</div></div></div><div class="slotActions">'+(it?'<button class="miniBtn gold upgradeBtn">強化</button>':'')+'</div>';if(it)c.querySelector(".upgradeBtn").addEventListener("click",()=>upgradeItem(it.id));grid.appendChild(c)});renderShop()}
...
function upgradeItem(id){const it=itemById(id),inv=state.inventory[id];if(!it||!inv)return;if(inv.upgrade>=5){toast("+5が最大です");return}const cost=upgradeCost(it);if(state.gold<cost){toast("GOLDが足りません（必要 "+cost+"G）");return}state.gold-=cost;inv.upgrade++;save();renderAll();toast(it.name+" を +"+inv.upgrade+" に強化")}
function renderShop(){const f=document.getElementById("shopFilters"),list=document.getElementById("shopList");f.innerHTML="";Object.keys(slotNames).forEach(slot=>{const b=document.createElement("button");b.className="shopFilter"+(currentShopFilter===slot?" active":"");b.textContent=slotIcons[slot]+" "+slotNames[slot];b.addEventListener("click",()=>{currentShopFilter=slot;renderShop()});f.appendChild(b)});list.innerHTML="";equipment.filter(it=>it.slot===currentShopFilter&&it.price>0).forEach(it=>{const owned=!!state.inventory[it.id],equipped=state.equipped[it.slot]===it.id,locked=state.level<it.minLv,z={hp:it.hp||0,atk:it.atk||0,def:it.def||0,crit:it.crit||0};const row=document.createElement("div");row.className="shopItem";row.innerHTML='<div class="itemArt art-'+it.slot+'"></div><div><div class="itemName rarity-'+it.rarity+'">'+it.name+'</div><div class="itemStats">'+formatBonus(z)+'</div><div class="small">'+rarityNames[it.rarity]+' / Tier '+it.tier+' / Lv.'+it.minLv+'〜</div></div><div style="text-align:right"><div class="price">🪙 '+it.price.toLocaleString()+'</div><button class="miniBtn '+(owned?'':'gold')+' buyBtn" '+(locked?'disabled':'')+'>'+(equipped?'装備中':owned?'装備':'購入')+'</button><div class="'+(locked?'locked':owned?'owned':'small')+'">'+(locked?'Lv.'+it.minLv+'必要':owned?'所持済み':'')+'</div></div>';const btn=row.querySelector(".buyBtn");if(equipped)btn.disabled=true;else btn.addEventListener("click",()=>{if(owned){state.equipped[it.slot]=it.id;save();renderAll();toast(it.name+" を装備") }else buyItem(it.id)});list.appendChild(row)})}
function buyItem(id){const it=itemById(id);if(!it)return;if(state.level<it.minLv){toast("Lv."+it.minLv+"から購入できます");return}if(state.gold<it.price){toast("GOLDが足りません");return}state.gold-=it.price;state.inventory[id]={owned:true,upgrade:0};state.equipped[it.slot]=id;save();renderAll();toast(it.name+" を購入・装備しました")}
function renderEditor(){const root=document.getElementById("menuEditor");root.innerHTML="";state.menu.forEach((ex,i)=>{const w=document.createElement("div");w.innerHTML='<div class="editor"><input class="n"><input class="k" inputmode="decimal"><input class="r" inputmode="numeric"><input class="s" inputmode="numeric"></div><div class="editrow"><select class="muscleSel"></select><button class="iconbtn del">削除</button></div>';w.querySelector(".n").value=ex.name;w.querySelector(".k").value=ex.sets[0]?.kg||0;w.querySelector(".r").value=ex.sets[0]?.reps||0;w.querySelector(".s").value=ex.sets.length;const sel=w.querySelector(".muscleSel");["STR","SHOULDER","BACK","CORE","LEG"].forEach(m=>{const o=document.createElement("option");o.value=m;o.textContent=m;o.selected=m===ex.muscle;sel.appendChild(o)});w.querySelector(".del").addEventListener("click",()=>{state.menu.splice(i,1);renderEditor()});root.appendChild(w)})}
document.getElementById("addExercise").addEventListener("click",()=>{state.menu.push({name:"新しい種目",muscle:"STR",sets:[{kg:10,reps:15},{kg:10,reps:15}]});renderEditor()});
document.getElementById("saveMenu").addEventListener("click",()=>{[...document.querySelectorAll("#menuEditor>div")].forEach((w,i)=>{const ex=state.menu[i],kg=+w.querySelector(".k").value||0,reps=+w.querySelector(".r").value||0,n=Math.max(1,+w.querySelector(".s").value||1);ex.name=w.querySelector(".n").value.trim()||"種目";ex.muscle=w.querySelector(".muscleSel").value;const old=ex.sets;ex.sets=Array.from({length:n},(_,j)=>({kg:old[j]?.kg??kg,reps:old[j]?.reps??reps}));ex.sets[0].kg=kg;ex.sets[0].reps=reps});save();toast("メニューを保存しました");renderTraining()});
...
document.getElementById("resetData").addEventListener("click",()=>{if(confirm("全記録を初期化しますか？")){state=clone(initial);starters.forEach(id=>state.inventory[id]={owned:true,upgrade:0});save();renderAll();renderEditor();renderTraining();toast("初期化しました")}});
